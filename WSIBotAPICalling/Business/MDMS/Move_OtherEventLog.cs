...
namespace SMDataMSSQLtoMySQLWJ.MDMS
{
    class Move_OtherEventLog
    {
        ...
        public Move_OtherEventLog()
        {
            try
            {
                SqlDataAdapter DAcmd = new SqlDataAdapter("select * from profiles_datamdms_sync_info where profile_Type='other_event_log_' and [read]='1'", mssqlcnn);
                        ...
                        for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
                        {
                            ...
                            if (MeterName != "")
                            {
                                ...
                                if (ds3 != null && ds3.Tables.Count > 0 && ds3.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    {
                                        Result = InsertSinglePhaseOtherEvent(ds2.Tables[0].Rows[k], MeterName, UtilityID);
                                    }
                                    else if (ds3.Tables[0].Rows[0]["meter_type_id"].ToString() == "1007")
                                    {
                                        Result = InsertThreePhaseOtherEvent(ds2.Tables[0].Rows[k], MeterName, UtilityID);
                                    }
            ...
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("OtherEvent_Main", ex.Message);
            }
            finally
            {
                mssqlcnn.Close();
        ...
        static string InsertSinglePhaseOtherEvent(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                ...
                if (k > 0)
                {
                    UpdateSyncinfo(dtServerTime, "other_event_log_");
                    return "Success";
                }
                else
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("OtherEvent_1p", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("OtherEvent_1p", ex.Message);
                return ex.Message;
            }
            finally
            {
        ...
        static string InsertThreePhaseOtherEvent(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                DateTime dtSource_Time = Convert.ToDateTime(InstRow["source_time_stamp"]);
                ...
                if (InstRow["P04"].ToString() != "")
                {
                    query = "INSERT INTO meter_event_log_three_phase VALUES('" + serial_number + "','" + InstRow["P02"] + "','" + dtSource_Time.ToString("yyyy-MM-dd HH:mm:ss") + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + InstRow["P03"] + "','" + InstRow["P04"] + "','" + InstRow["P05"] + "','" + InstRow["P06"] + "','" + InstRow["P07"] + "','" + InstRow["P08"] + "',NULL,NULL,'" + InstRow["P09"] + "','" + InstRow["P10"] + "','" + InstRow["P11"] + "','" + InstRow["P12"] + "','" + InstRow["P14"] + "','" + InstRow["P15"] + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + UtilityID + "')";
                }
                else
                {
                    query = "INSERT INTO meter_event_log_three_phase VALUES('" + serial_number + "','" + InstRow["P02"] + "','" + dtSource_Time.ToString("yyyy-MM-dd HH:mm:ss") + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'" + InstRow["P03"] + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + UtilityID + "')";
                }
                MySqlCommand cmd = new MySqlCommand(query, mysqlcnn);
                if (mysqlcnn.State == System.Data.ConnectionState.Closed)
                {
                ...
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("OtherEvent_3p", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("OtherEvent_3p", ex.Message);
                return ex.Message;
            }
            finally
            {
