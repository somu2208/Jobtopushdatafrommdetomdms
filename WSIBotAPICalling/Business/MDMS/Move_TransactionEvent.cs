...
namespace SMDataMSSQLtoMySQLWJ.MDMS
{
    class Move_TransactionEvent
    {
        ...
        public Move_TransactionEvent()
        {
            try
            {
                SqlDataAdapter DAcmd = new SqlDataAdapter("select * from profiles_datamdms_sync_info where profile_Type='transaction_event_log_' and [read]='1'", mssqlcnn);
                        ...
                        for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
                        {
                            ...
                            if (MeterName != "")
                            {
                                ...
                                if (ds3 != null && ds3.Tables.Count > 0 && ds3.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    {
                                        Result = InsertSinglePhaseTranctionEvent(ds2.Tables[0].Rows[k], MeterName, UtilityID);
                                    }
                                    else if (ds3.Tables[0].Rows[0]["meter_type_id"].ToString() == "1007")
                                    {
                                        Result = InsertThreePhaseTransactionEvent(ds2.Tables[0].Rows[k], MeterName, UtilityID);
                                    }
            ...
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Transaction_Main", ex.Message);
            }
            finally
            {
                mssqlcnn.Close();
        ...
        static string InsertSinglePhaseTranctionEvent(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                ...
                if (k > 0)
                {
                    UpdateSyncinfo(dtServerTime, "transaction_event_log_");
                    return "Success";                    
                }
                else
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Transaction_1p", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Transaction_1p", ex.Message);
                return ex.Message;
            }
            finally
            {
        ...
        static string InsertThreePhaseTransactionEvent(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                DateTime dtSource_Time = Convert.ToDateTime(InstRow["source_time_stamp"]);
                ...
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Transaction_3p", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Transaction_3p", ex.Message);
                return ex.Message;
            }
            finally
            {
