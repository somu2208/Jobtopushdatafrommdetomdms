...
namespace SMDataMSSQLtoMySQLWJ.MDMS
{
    class Move_Blockload
    {
        ...
        public Move_Blockload()
        {
            try
            {
                SqlDataAdapter DAcmd = new SqlDataAdapter("select * from profiles_datamdms_sync_info where profile_Type='blockload_profile_' and [read]='1'", mssqlcnn);
                        ...
                        for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
                        {
                            ...
                            if (MeterName != "")
                            {
                                ...
                                if (ds3 != null && ds3.Tables.Count > 0 && ds3.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    {
                                        Result = InsertSinglePhaseBlockLoad(ds2.Tables[0].Rows[k], MeterName, UtilityID);
                                    }
                                    else if (ds3.Tables[0].Rows[0]["meter_type_id"].ToString() == "1007")
                                    {
                                        Result = InsertThreePhaseBlockLoad(ds2.Tables[0].Rows[k], MeterName, UtilityID);
                                    }
            ...
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Blockload_Main", ex.Message);
        ...
        static string InsertSinglePhaseBlockLoad(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("INSERT INTO meter_block_load_profile_single_phase VALUES('" + serial_number + "','" + dtSource_Time.ToString("yyyy-MM-dd HH:mm:ss") + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + InstRow["P02"] + "','" + InstRow["P03"] + "','" + InstRow["P05"] + "','" + InstRow["P04"] + "','" + InstRow["P06"] + "','" + InstRow["P07"] + "','" + InstRow["P08"] + "','" + InstRow["P09"] + "','" + UtilityID + "',NULL,NULL)", mysqlcnn);
                ...
                if (k > 0)
                {
                    UpdateSyncinfo(dtServerTime, "blockload_profile_");
                    return "Success";
                }
                else
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Blockload_1p", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Blockload_1p", ex.Message);
                return ex.Message;
            }
            finally
            {
        ...
        static string InsertThreePhaseBlockLoad(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                DateTime dtSource_Time = Convert.ToDateTime(InstRow["source_time_stamp"]);
                DateTime dtServerTime = Convert.ToDateTime(InstRow["server_time"]);
                MySqlCommand cmd = new MySqlCommand("INSERT INTO meter_block_load_profile_three_phase VALUES('" + serial_number + "','" + dtSource_Time.ToString("yyyy-MM-dd HH:mm:ss") + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + InstRow["P02"] + "',NULL,'" + InstRow["P08"] + "','" + InstRow["P10"] + "','" + InstRow["P09"] + "','" + InstRow["P11"] + "','" + InstRow["P03"] + "','" + InstRow["P04"] + "','" + InstRow["P05"] + "','" + InstRow["P06"] + "','" + InstRow["P07"] + "',NULL,'" + InstRow["P13"] + "','" + InstRow["P12"] + "','" + UtilityID + "','" + InstRow["P14"] + "','" + InstRow["P15"] + "','" + InstRow["P16"] + "','" + InstRow["P17"] + "',NULL,NULL)", mysqlcnn);
                if (mysqlcnn.State == System.Data.ConnectionState.Closed)
                {
                    mysqlcnn.Open();
                }
                ...
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Blockload_3p", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Blockload_3p", ex.Message);
                return ex.Message;
            }
            finally
            {
