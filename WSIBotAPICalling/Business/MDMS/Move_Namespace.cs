...
namespace SMDataMSSQLtoMySQLWJ.MDMS
{
    class Move_Namespace
    {
        ...
        public Move_Namespace()
        {
            try
            {
                SqlDataAdapter DAcmd = new SqlDataAdapter("select * from profiles_datamdms_sync_info where profile_Type='nameplate_profile_' and [read]='1'", mssqlcnn);
                        ...
                        for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
                        {
                            ...
                            if (MeterName != "")
                            {
                                ...
                                if (ds3 != null && ds3.Tables.Count > 0 && ds3.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    Result = InsertNamespace(ds2.Tables[0].Rows[k], MeterName, UtilityID);                                    
                                }
                            }
                        }
                    }
            ...
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Namespace_Main", ex.Message);
            }
            finally
            {
                mssqlcnn.Close();
        ...
        static string InsertNamespace(DataRow InstRow, string serial_number,string UtilityID)
        {
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("INSERT INTO meter_name_plate VALUES('" + serial_number + "','" + dtSource_Time.ToString("yyyy-MM-dd HH:mm:ss") + "','" + dtServerTime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + InstRow["P02"] + "','" + InstRow["P03"] + "','" + InstRow["P04"] + "','" + InstRow["P05"] + "','" + InstRow["P06"] + "','" + InstRow["P07"] + "','" + InstRow["P08"] + "','"+ UtilityID + "')", mysqlcnn);
                ...
                if (k > 0)
                {
                    UpdateSyncinfo(dtServerTime, "nameplate_profile_");
                    return "Success";
                }
                else
                {
                    Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Namespace", "Fail");
                    return "Fail";
                }
            }
            catch (Exception ex)
            {
                Logs.Logging_Transaction message_log = new Logs.Logging_Transaction("Namespace", ex.Message);
                return ex.Message;
            }
            finally
            {
